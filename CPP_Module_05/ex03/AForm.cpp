...
AForm::AForm(const std::string sName, const std::string sTarget, int sReqGradeSign, int sReqGradeExe) : name(sName), target(sTarget),
...
AForm::AForm(const AForm& bcopy) : name(bcopy.name), target(bcopy.target), reqGradeSign(bcopy.reqGradeSign), reqGradeExe(bcopy.reqGradeExe)
{
	*this = bcopy;
